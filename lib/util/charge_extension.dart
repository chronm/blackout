...
extension ChargeExtension on Charge {
  LocalDate get creationDate {
    return modelChanges
        .firstWhere((element) => element.modification == ModelChangeType.create)
        .modificationDate;
  }

  String get scientificAmount => UnitConverter.toScientific(Amount(
          amount,
          Unit.fromSi(
              product.group != null ? product.group.unit : product.unit)))
      .toString();

  UnitEnum get unit => product.unit;

  double get amount => changes.length != 0
      ? changes.map((c) => c.value).reduce((a, b) => a + b)
      : 0;
  ...
  bool get warn {
    if (product.group?.warnInterval != null) {
      return expirationDate != null
          ? expirationDate.subtract(product.group.warnInterval) <=
              LocalDate.today()
          : false;
    }
    if (product.warnInterval != null) {
      return expirationDate != null
          ? expirationDate.subtract(product.warnInterval) <= LocalDate.today()
          : false;
    }
    return false;
  }
